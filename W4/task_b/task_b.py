...
class ImageEmbeddingLayer(torch.nn.Module):
    def __init__(self):
        ...
        self.image_linear = torch.nn.Linear(4096, 384)
        self.final_linear = torch.nn.Linear(384, 256)
...
class TextEmbeddingLayer(torch.nn.Module):
    def __init__(self):
        ...
        self.text_linear = torch.nn.Linear(300, 384)
        self.final_linear = torch.nn.Linear(384, 256)
        self.activation = torch.nn.ReLU()
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
...
class Model():
    ...
    def train_img_to_text(self, dataloader, n_epochs=4, save_path_img='./weights/image_model_task_b_1epoch_embed_256.pth', save_path_txt='./weights/text_model_task_b_1epoch_embed_256.pth') -> None:
        # Embedding Models
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx, (anchor_img, captions, _) in pbar:
                ...
                text_embds = self.text_model.forward(text_vecs) # (bs, 4096)

...
class ImageEmbeddingLayer(torch.nn.Module):
    def __init__(self):
        ...
        self.image_linear = torch.nn.Linear(4096, 512)
        self.final_linear = torch.nn.Linear(512, 256)
    ...
    def train_img_to_text(self, dataloader, n_epochs=1) -> None:
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx, (anchor_img, captions, _) in pbar:
                ...
                anchor_out = self.model_img(anchor_img) # (bs, 256)
                ...
                text_embds = self.text_model.forward(sentence_embedding)

                neg_embds = torch.roll(anchor_out, shifts=-1, dims=0) # (bs, 256)

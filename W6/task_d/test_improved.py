...
def myDataLoader(train_data, valid_data, test_data, NUM_WORKERS, BATCH_SIZE, BATCH_SIZE_VALID, BATCH_SIZE_TEST):
    ...
    augmented_dataloader = DataLoader(dataset=train_data, 
                                batch_size=BATCH_SIZE, # how many samples per batch?
                                num_workers=NUM_WORKERS,
                                shuffle=True)
...
def my_test_step(model: torch.nn.Module, dataloader: torch.utils.data.DataLoader, test_data):
    ...
    # IMPROVE TESTING, VOTING AND SUMMING PROBS
    #=====================================================
    # APART FROM PREDICTIONS SAVE PROBABILITY, FOR SUMMING 
    # THE PROBABILITIES OF EACH CLASS OF THE SAME USER ID
    predictions_and_probabilities = [['VideoName','ground_truth','prediction', "probabilities"]]
    #=====================================================
    ...
    for f in test_data.imgs:
        ...
        # IMPROVE TESTING
        predictions_and_probabilities.append([f_name, cat, '-1', '-1'])
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        # IMPROVE TESTING
        #========================================
        predictions_and_probabilities[counter][2] = y_pred_class.tolist()[0]+1 # y_pred_class.tolist()[0] from 0 to 6
        predictions_and_probabilities[counter][3] = str(probabilities_list)
        #========================================
    ...
    with open(csv_file, 'r') as file:
        ...
        for row in csv_reader:
            ...
            sum_probabilities(video_probabilities,video_truth,video_name, probabilities,gt)
...
def sum_probabilities(video_probabilities,video_truth,video_name, probabilities,truth):

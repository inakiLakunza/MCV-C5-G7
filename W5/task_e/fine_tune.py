...
class Model():
    def __init__(self, weights_txt, weights_img):
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        ...
        # LOAD PRE-TRAINED WEIGHTS FROM WEEK 4
        #==========================================================
        print("Loading weights...")
        self.model_img.load_state_dict(torch.load(weights_img,map_location=torch.device('cpu')))
        self.text_model.load_state_dict(torch.load(weights_txt,map_location=torch.device('cpu')))
        #==========================================================

    def train_img_to_text(self, dataloader, weights_name, n_epochs=5) -> None:
        # Embedding Models
        ...
        for epoch in range(n_epochs):
            ...
            for batch_idx, (anchor_img, captions, _) in pbar:
                ...
                # Image Embeddings
                anchor_img = anchor_img.float().to(self.device) # (bs, 3, 224, 224)
        ...
        # SAVE EMBEDDINGS
        save_path_img = f'./weights_all_5/image_model_task_e_1epoch_{weights_name}.pth'
        save_path_txt = f'./weights_all_5/text_model_task_e_1epoch_{weights_name}.pth'
        os.makedirs("./weights_all_5", exist_ok=True)
...
if __name__ == '__main__':
    ...
    model = Model(
        weights_txt='/ghome/group07/C5-W4/task_c/weights/text_model_task_a_1epoch.pth', 
        weights_img='/ghome/group07/C5-W4/task_c/weights/image_model_task_a_1epoch.pth'
    )
    print("Loading data...")
    ...
    dataset_2_1 = Augmented_2_1(root_dir=root_dir, json_dir=json_dir)
    dataloader_2_1 = torch.utils.data.DataLoader(dataset_2_1, batch_size=32,  drop_last=True, shuffle=True)
    ...
    dataset_Turbo = Augmented_Turbo(root_dir=root_dir, json_dir=json_dir)
    dataloader_turbo = torch.utils.data.DataLoader(dataset_Turbo, batch_size=32,  drop_last=True, shuffle=True)

    root_dir = '/ghome/group07/C5-W5/task_d/augmented_images/'  
    json_dir = '/ghome/group07/C5-W5/task_d/augmented_dataset.json'     
    dataset_Turbo_all = Augmented_Turbo(root_dir=root_dir, json_dir=json_dir)
    dataset_Turbo_all = torch.utils.data.DataLoader(dataset_Turbo_all, batch_size=32,  drop_last=True, shuffle=True)

    model.train_img_to_text(dataloader_2_1, weights_name='2_1')
    model.train_img_to_text(dataloader_turbo, weights_name='turbo_100_steps')
    model.train_img_to_text(dataset_Turbo_all, weights_name='All')

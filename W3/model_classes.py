...
class ResNet_embedding(torch.nn.Module):
    def __init__(self, embed_size=2096, batch_size=8, loss=torch.nn.CrossEntropyLoss(), base_lr=1e-4):
        super().__init__()
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        ...
        self.lr_scheduler = optim.lr_scheduler.StepLR(self.optimizer, step_size=2, gamma=0.9)
    ...
    def extract_features_with_captions(self, dataloader):
        features = []
        targets = []
        text_targets = []
        for batch_idx, (images, labels, captions) in enumerate(dataloader):
            ...
            features.append(outputs.cpu().detach().numpy())
            targets.append(labels.cpu().numpy())
            text_targets.append(np.array(captions))
        return np.concatenate(features, axis=0), np.concatenate(targets, axis=0), np.concatenate(text_targets, axis=0)
    
    def train_model(self, train_dataloader, n_epochs=50):
        data_labels = [(x['label'].tolist()) for x in train_dataloader.dataset]
        ...
        metric_trainer = trainers.MetricLossOnly(
            ...
            lr_schedulers={"trunk_scheduler_by_epoch": self.lr_scheduler},
    ...
    def train_model_with_dataloader(self, train_dataloader, n_epochs=50):
        wrapped_dataloader = WrapperDataloader(train_dataloader.dataset)
        data_labels = [item[1].item() for _, item in enumerate(wrapped_dataloader)]
        class_sampler = samplers.MPerClassSampler(
            labels= data_labels,
            m=16 // 8,
            batch_size=16,
            length_before_new_iter=len(wrapped_dataloader),
        )
        # change the format here, train_dataloader is like this:
    ...
    def test(self, labels_train, dataloader_test, model_knn):
        ...
        neighbors = model_knn.kneighbors(features_test, return_distance=False)
        neighbors_labels = []
        ...
        for batch_idx, batch in enumerate(dataloader_test):
            ...
            cv2_image = cv.cvtColor(cv2_image, cv.COLOR_BGR2RGB)
            cv.imwrite('/ghome/group07/C5-W3/images_a/image_test.jpg',cv2_image*255)
            break
   


        print(neighbors[0])
        print(neighbors_labels[0])
        
        for batch_idx, batch in enumerate(dataloader_train):
            images, labels, captions = batch
            for j in range(len(neighbors[0])):
                sample_image = images[neighbors[0][j]]    
                numpy_image = sample_image.numpy()
                cv2_image = np.transpose(numpy_image, (1, 2, 0))
                cv2_image = cv.cvtColor(cv2_image, cv.COLOR_BGR2RGB)
                cv.imwrite('/ghome/group07/C5-W3/images_a/image'+str(j)+'.jpg',cv2_image*255)
